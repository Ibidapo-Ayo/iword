...
const searchWord = async () => {
  ...
  const checkForNumber = /^\d+$/;

  if (words.value.trim() === "") {
    alert("Enter word...");
  } else {
    if (!checkForNumber.test(words.value)) {
      words.value = "";
   ...
   <a href="${
    ...
    `;
        ...
        .catch(err => {
          console.log(err);
          document.getElementById("ovalSpinner").classList.add("hidden");
          // document.getElementById("content").innerHTML = "An error Occured";
        });
      ...
      alert("Can't search for Number, please enter text...");
      words.value =""
    }
  }
};
